...
function socket(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('joined-user', (data) => {
            ...
            io.to(data.roomName).emit('online-users', getUsers(user[data.roomName]))
        })

        socket.on('chat', (data) => {
            io.to(data.roomName).emit('chat', {users:data.userName, message: data.message})
        })
