...
const MAXIMUM_NUMBERS_AFTER_DECIMAL_POINTS = 5
...
function setOperator(op) {
    ...
    if (firstNumber.length > 0 && secondNumber.length > 0 && operation){
        ...
        firstNumber = parseFloat(firstNumber).toFixed(MAXIMUM_NUMBERS_AFTER_DECIMAL_POINTS)
        // let split = firstNumber.split('.')
        // if (split.length === 2) {
        //     if (split[1].length > MAXIMUM_NUMBERS_AFTER_DECIMAL_POINTS) {
        //         split[1] = parselFloat(split[1]).toFixed().toString()
        //     }
        // }
        result.innerText = firstNumber
...
function setCommaEnebled(enabled) {
